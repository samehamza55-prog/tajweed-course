...
.themeBtn{width:44px!important;height:44px!important;margin:0!important;padding:0!important;border-radius:14px!important;background:#eff7f3!important;color:#174d40!important;box-shadow:none!important;font-size:20px!important}
.achievement-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin-top:14px}
.achievement{padding:15px 10px;text-align:center;border:1px solid #e8dfbd;border-radius:20px;background:linear-gradient(145deg,#fffdf5,#fff);box-shadow:0 10px 25px #6d572a0d}
.achievement .ico{font-size:28px}.achievement b{display:block;margin-top:6px;font-size:12px}.achievement small{display:block;color:var(--muted);margin-top:4px;font-size:10px;line-height:1.5}
.chart-card{margin-top:18px;padding:18px;border:1px solid var(--line);border-radius:23px;background:linear-gradient(145deg,#fbfdfc,#fff)}
.chart-title{display:flex;justify-content:space-between;gap:8px;align-items:center}.chart-title b{font-size:17px}.chart-wrap{margin-top:12px;overflow-x:auto}
.chart-svg{width:100%;min-width:560px;height:245px;display:block}
.certificate-btn{background:linear-gradient(135deg,#b28732,#d7b86a,#a77825)!important;color:#fff!important}
.action-row{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:12px}.action-row button{margin-top:0}
.dark{--ink:#eaf7f2;--muted:#a9c0b8;--bg:#081914;--white:#10251f;--line:#27433a;--shadow:0 24px 70px rgba(0,0,0,.28)}
body.dark{background:radial-gradient(circle at 5% 0,#0d3a2d 0,transparent 30%),radial-gradient(circle at 100% 7%,#211a38 0,transparent 27%),linear-gradient(135deg,#06110e,#0a1c17)}
body.dark .top,body.dark .card,body.dark .result,body.dark .stat,body.dark .chart-card,body.dark .achievement{background:#10231eeF;border-color:#29463d}
body.dark input{background:#0c1b17;color:#eaf7f2;border-color:#2a4a40}body.dark th{background:#0c1b17;color:#9db4ac}
body.dark .secondary{background:#1a3029;color:#d9eee8}.dark .hint{background:#0e201b}
@media(max-width:820px){.achievement-grid{grid-template-columns:repeat(2,1fr)}}
@media(max-width:480px){.achievement-grid{grid-template-columns:repeat(2,1fr)}.action-row{grid-template-columns:1fr}}
@media print{
 body>*:not(#certificatePrint){display:none!important}
 #certificatePrint{display:block!important}
}
#certificatePrint{display:none}
...
<body>
<main class="wrap">
<header class="top"><div class="brand"><div class="logo">✦</div><div><b>بوابة دورة التجويد</b><small>نتائج • متابعة • إنجاز</small></div></div><div style="display:flex;gap:8px;align-items:center"><button class="themeBtn" onclick="toggleTheme()" title="تغيير المظهر">☾</button><div class="status">● النظام جاهز وآمن</div></div></header>
...
function badgeList(pct,scores){
 const badges=[];
 if(pct>=90) badges.push(['🏆','نجم الإتقان','نتيجة إجمالية 90%+']);
 if(pct>=75) badges.push(['🌟','طالب متميز','نتيجة إجمالية 75%+']);
 if(scores.length>=2) badges.push(['🔥','مستمر','أكثر من اختبار']);
 if(scores.length>=1 && scores.every(x=>Number(x.score||0)>=Number(x.max||0)*.5)) badges.push(['📚','مواظب','لا توجد نتيجة أقل من 50%']);
 if(scores.some(x=>Number(x.score||0)===Number(x.max||0))) badges.push(['💎','علامة كاملة','حقق العلامة الكاملة']);
 if(!badges.length) badges.push(['🌱','بداية طيبة','استمر وطور مستواك']);
 return badges.slice(0,4);
}
function makeChart(scores){
 if(!scores.length)return '<div class="chart-card"><b>📈 تطور النتائج</b><p class="date">لا توجد اختبارات لعرضها بعد.</p></div>';
 const W=720,H=230,pad=42,maxY=100;
 const pts=scores.map((x,i)=>{const pct=x.max?Math.round(Number(x.score||0)/Number(x.max)*100):0;const xx=pad+(scores.length===1?0:(i*(W-pad*2)/(scores.length-1)));const yy=H-pad-(pct*(H-pad*2)/maxY);return {x:xx,y:yy,p:pct}});
 const points=pts.map(a=>`${a.x},${a.y}`).join(' ');
 const circles=pts.map((a,i)=>`<circle cx="${a.x}" cy="${a.y}" r="5" fill="#087b5b"/><text x="${a.x}" y="${a.y-11}" text-anchor="middle" font-size="11" font-weight="800" fill="#087b5b">${a.p}%</text><text x="${a.x}" y="${H-12}" text-anchor="middle" font-size="10" fill="currentColor">${i+1}</text>`).join('');
 const grids=[0,25,50,75,100].map(v=>{const y=H-pad-(v*(H-pad*2)/100);return `<line x1="${pad}" x2="${W-pad}" y1="${y}" y2="${y}" stroke="#dce8e3" stroke-width="1"/><text x="10" y="${y+4}" font-size="10" fill="#70817c">${v}%</text>`}).join('');
 return `<div class="chart-card"><div class="chart-title"><b>📈 تطور النتائج</b><span class="badge">آخر ${scores.length} اختبار</span></div><div class="chart-wrap"><svg class="chart-svg" viewBox="0 0 ${W} ${H}" role="img" aria-label="رسم بياني لتطور النتائج">${grids}<polyline points="${points}" fill="none" stroke="#087b5b" stroke-width="4" stroke-linecap="round" stroke-linejoin="round"/>${circles}</svg></div><div class="date">الأرقام أسفل الرسم تمثل ترتيب الاختبارات من الأقدم إلى الأحدث.</div></div>`;
}
function openCertificate(s,pct,total,max){
 const avg=max?Math.round(total/max*100):0;
 const w=window.open('','_blank','width=900,height=700');
 if(!w){alert('اسمح للنوافذ المنبثقة ثم جرّب مرة أخرى.');return}
 w.document.write(`<!doctype html><html lang="ar" dir="rtl"><head><meta charset="utf-8"><title>شهادة إنجاز - ${esc(s.name)}</title><style>
 body{margin:0;background:#f2f0e8;font-family:Tahoma,Arial,sans-serif;color:#173b31}.cert{width:min(900px,92vw);min-height:620px;margin:30px auto;padding:12px;background:linear-gradient(135deg,#d6b766,#fff8dc,#a9822f);box-sizing:border-box}.inner{min-height:596px;background:#fffdf7;border:3px solid #b28a35;outline:1px solid #ead7a0;padding:48px 55px;box-sizing:border-box;text-align:center;position:relative}.orn{font-size:42px;color:#b28a35}.small{color:#70817c;font-size:15px}.title{font-size:38px;margin:15px 0;color:#8b6720}.name{font-size:34px;font-weight:900;margin:25px 0;color:#173b31}.score{font-size:20px;margin:18px 0}.quote{margin:25px auto;max-width:650px;line-height:2;color:#4f6b62}.stamp{width:150px;max-height:150px;object-fit:contain;margin:8px auto}.footer{margin-top:18px;font-size:13px;color:#70817c}@media print{body{background:white}.cert{width:100%;min-height:100vh;margin:0}.inner{min-height:calc(100vh - 24px)}} </style></head><body><div class="cert"><div class="inner"><div class="orn">✦ ✦ ✦</div><div class="small">بوابة دورة التجويد</div><div class="title">شهادة إنجاز وتقدّم</div><div class="small">تشهد إدارة الدورة بأن الطالب</div><div class="name">${esc(s.name)}</div><div class="score">قد أتم متابعة الاختبارات بنسبة إجمالية <b>${avg}%</b><br>بمجموع <b>${total} / ${max}</b></div><div class="quote">بارك الله في جهدك، واجعل العلم طريقًا للإتقان، وواصل التقدم بثبات وهمّة.</div><img class="stamp" src="/khatm_sami.png" alt="ختم سامي"><div class="footer">ختم وتوقيع • سامي عدي &nbsp; | &nbsp; الإتقان هوية النجاح</div></div></div><script>setTimeout(()=>window.print(),500)<\/script></body></html>`);
 w.document.close();
}
function studentLogin(){
 ...
 const badges=badgeList(pct,s.scores).map(b=>`<div class="achievement"><div class="ico">${b[0]}</div><b>${b[1]}</b><small>${b[2]}</small></div>`).join('');
 ...
 document.getElementById('student').classList.remove('hidden');document.getElementById('student').innerHTML=`<div class="student-head"><div><div class="kicker">السلام عليكم ورحمة الله وبركاته</div><h2>نتائج الطالب: <span class="green">${esc(s.name)}</span></h2></div><button class="secondary" onclick="logoutStudent()" style="width:auto">خروج</button></div><div class="stats"><div class="stat"><small>النسبة الإجمالية</small><strong class="green">${pct}%</strong></div><div class="stat"><small>المجموع الكلي</small><strong class="blue">${total} / ${max}</strong></div><div class="stat"><small>عدد الاختبارات</small><strong class="gold">${s.scores.length}</strong></div></div><div class="action-row"><button class="certificate-btn" onclick='openCertificate(${JSON.stringify(s)},${pct},${total},${max})'>📜 إصدار شهادة إنجاز</button><button class="secondary" onclick="window.print()">🖨️ طباعة النتائج</button></div><div class="smart"><small>✦ المساعد الذكي للتقدم</small><h3>${label}</h3><p>${advice}</p></div><div class="section-title">🏅 إنجازاتك وشاراتك</div><div class="achievement-grid">${badges}</div>${makeChart(s.scores)}<div class="section-title">نتائج الاختبارات</div>${results}<div class="msg">✨ أحسنت، استمر في المراجعة والتدريب؛ فالإتقان يأتي مع الصبر والمداومة.</div><div class="stamp-wrap"><img class="stamp" src="/khatm_sami.png" alt="ختم سامي والتوقيع"></div>`;
 document.getElementById('pw').value='';document.getElementById('student').scrollIntoView({behavior:'smooth'})
}
function toggleTheme(){
 const on=document.body.classList.toggle('dark');localStorage.setItem('tajweed_theme',on?'dark':'light');
 document.querySelector('.themeBtn').textContent=on?'☀':'☾';
}
(function(){if(localStorage.getItem('tajweed_theme')==='dark'){document.body.classList.add('dark');setTimeout(()=>{const b=document.querySelector('.themeBtn');if(b)b.textContent='☀'},0)}})();
...
function addStudent(){
 ...
 const password=document.getElementById('newStudentPw').value.trim();
 if(!name||!password){alert('أدخل اسم الطالب وكلمة المرور.');return}
 if(state.some(s=>s.password===password)){alert('كلمة المرور مستخدمة بالفعل، اختر كلمة أخرى.');return}
 const next=state.length+1,id='S'+String(next).padStart(3,'0');
 const scores=(state[0]?.scores||[]).map(x=>({name:x.name,date:x.date,score:0,max:x.max}));
 state.push({id,name,password,scores});save();renderAdmin();alert('تمت إضافة الطالب بنجاح ✓');
}
